...
namespace DataFormats_EgammaReco {
  struct dictionary {
    ...
    std::pair<const DetId,float> thepair;   
    std::map<DetId,float> themap; 
    edm::Wrapper<std::map<DetId,float> > themap1;
    std::map<DetId, std::pair<unsigned int, unsigned int> > dummydens1;
    std::map<DetId, std::pair<unsigned long, unsigned long> > dummydens2;
