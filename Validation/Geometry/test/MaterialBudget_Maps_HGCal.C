...
// Main
void MaterialBudget_Maps_HGCal() {
	...
	TString subDetectorFileName = "matbdg_HGCal.root";
	...
	//case t/X0
	TProfile2D *prof_x0_HGCal_XY = (TProfile2D*)subDetectorFile->Get("30");
	TH2D* hist_x0_HGCal_XY = (TH2D*)prof_x0_HGCal_XY->ProjectionXY();
...
//	can_x0.Range(0,0,25,25);
	...
	// Draw
	hist_x0_HGCal_XY->GetXaxis()->SetRangeUser(xmin,xmax);
	hist_x0_HGCal_XY->GetYaxis()->SetRangeUser(ymin,ymax);
	hist_x0_HGCal_XY->Rebin2D(rebin_x0_x,rebin_x0_y);
	hist_x0_HGCal_XY->Scale(1/norm_x0);

	zmax_x0 = hist_x0_HGCal_XY->GetMaximum();
	hist_x0_HGCal_XY->SetMinimum(zmin_x0);
	hist_x0_HGCal_XY->SetMaximum(zmax_x0);

	hist_x0_HGCal_XY->GetXaxis()->SetTitle("#eta");
	hist_x0_HGCal_XY->GetYaxis()->SetTitle("#varphi [rad]");
	hist_x0_HGCal_XY->GetZaxis()->SetTitle("t/X_{0}");
	hist_x0_HGCal_XY->GetZaxis()->SetTitleOffset(1.1);

	hist_x0_HGCal_XY->Draw("zcol");
...
//----------------
	...
	//case t/lambdaI
	TProfile2D *prof_lambdaI_HGCal_XY = (TProfile2D*)subDetectorFile->Get("1030");
	TH2D* hist_lambdaI_HGCal_XY = (TH2D*)prof_lambdaI_HGCal_XY->ProjectionXY();
...
//	can_lambdaI.Range(0,0,25,25);
	...
	// Draw
	hist_lambdaI_HGCal_XY->GetXaxis()->SetRangeUser(xmin,xmax);
	hist_lambdaI_HGCal_XY->GetYaxis()->SetRangeUser(ymin,ymax);
	hist_lambdaI_HGCal_XY->Rebin2D(rebin_lambdaI_x,rebin_lambdaI_y);
	hist_lambdaI_HGCal_XY->Scale(1/norm_lambdaI);

	zmax_lambdaI = hist_lambdaI_HGCal_XY->GetMaximum();
	hist_lambdaI_HGCal_XY->SetMinimum(zmin_lambdaI);
	hist_lambdaI_HGCal_XY->SetMaximum(zmax_lambdaI);

	hist_lambdaI_HGCal_XY->GetXaxis()->SetTitle("#eta");
	hist_lambdaI_HGCal_XY->GetYaxis()->SetTitle("#varphi [rad]");
	hist_lambdaI_HGCal_XY->GetZaxis()->SetTitle("t/#lambda_{I}");

	hist_lambdaI_HGCal_XY->Draw("zcol");
