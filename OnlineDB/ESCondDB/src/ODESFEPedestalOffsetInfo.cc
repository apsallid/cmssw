...
int ODESFEPedestalOffsetInfo::fetchNextId()  throw(std::runtime_error) {
  ...
  try {
    ...
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::fetchNextId():  "+e.getMessage()));
  }
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::prepareWrite():  "+e.getMessage()));
  }
...
void ODESFEPedestalOffsetInfo::setParameters(const std::map<std::string,std::string>& my_keys_map){
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::writeDB():  "+e.getMessage()));
  }


  // Now get the ID
  if (!this->fetchID()) {
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::writeDB:  Failed to write"));
  } else {
  ...
  throw(std::runtime_error)
{
  ...
  if(result->getId()==0 && (result->getConfigTag()=="") ){
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::fetchData(): no Id defined for this ODESFEPedestalOffsetInfo "));
  }
  ...
  try {
    if(result->getId()!=0) {
      ...
      throw(std::runtime_error("ODESFEPedestalOffsetInfo::fetchData(): no Id defined for this record "));
    }
    ...
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::fetchData():  "+e.getMessage()));
  }
}
...
int ODESFEPedestalOffsetInfo::fetchID()    throw(std::runtime_error)
{
  ...
  try {
    ...
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::fetchID:  "+e.getMessage()));
  }
...
std::map< int,int >  ODESFEPedestalOffsetInfo::fetchIovId(const int runp,const int runm) throw(std::runtime_error)
{
  ...
  try {
    ...
    throw(std::runtime_error("ODESFEPedestalOffsetInfo::fetchRunPlus:  "+e.getMessage()));
 }
 return  fillMap ;
}
...
std::map< int,int >  ODESFEPedestalOffsetInfo::fetchGain(const std::string gain)    throw(std::runtime_error)
{
  std::map< int,int > fillMap;
