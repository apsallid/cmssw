  ...
  throw(std::runtime_error)
{
  ...
  try {
    m_readStmt->setSQL("SELECT * FROM " + getTable() + " WHERE rec_id = :rec_id order by fed_id, fiber_id, kchip_id, pace_id, strip_id, rx_id ");
    ...
    throw(std::runtime_error("ODESFEPedestalOffsetsDat::fetchData():  "+e.getMessage()));
  }
}

//  ************************************************************************   // 
/*
void ODESFEPedestalOffsetsDat::fetchDataFromCondDB(std::vector< ODESFEPedestalOffsetsDat >* p, ODESFEPedestalOffsetInfo* iov)
  throw(std::runtime_error)
{
