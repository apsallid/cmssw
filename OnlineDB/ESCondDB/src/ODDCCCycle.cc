  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO ES_DCC_Cycle (cycle_id, dcc_configuration_id ) "
		 "VALUES (:1, :2 )");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT cycle_id, dcc_configuration_id FROM es_dcc_cycle "
		 "WHERE cycle_id = :1 ");
    stmt->setInt(1, m_ID);
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT  dcc_configuration_id FROM es_dcc_cycle "
		 "WHERE cycle_id = :1 ");
