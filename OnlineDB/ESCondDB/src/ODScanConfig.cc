...
#include "OnlineDB/ESCondDB/interface/ODScanConfig.h"
...
int ODScanConfig::fetchNextId()  throw(std::runtime_error) {
  ...
  try {
    ...
    m_readStmt->setSQL("select es_scan_config_sq.NextVal from dual");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO es_scan_dat ( scan_id, scan_tag ,"
...
int ODScanConfig::fetchID()    throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT scan_id FROM es_scan_dat "
