  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT def_id FROM es_run_mode_def WHERE "
		 "run_mode_string   = :1"
		 );
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT run_mode_string FROM es_run_mode_def WHERE def_id = :1");
    stmt->setInt(1, id);
  ...
  throw(std::runtime_error)
{
  ...
  try {
    Statement* stmt = m_conn->createStatement();
    stmt->setSQL("SELECT def_id FROM es_run_mode_def ORDER BY def_id");
    ResultSet* rset = stmt->executeQuery();
