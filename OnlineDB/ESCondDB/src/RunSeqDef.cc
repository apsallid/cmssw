...
#include "OnlineDB/ESCondDB/interface/RunSeqDef.h"
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT def_id FROM es_sequence_type_def WHERE "
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT run_type_def_id, sequence_type_string FROM es_sequence_type_def WHERE def_id = :1");
    stmt->setInt(1, id);
  ...
  throw(std::runtime_error)
{
  ...
  try {
    Statement* stmt = m_conn->createStatement();
    stmt->setSQL("SELECT def_id FROM es_sequence_type_def ORDER BY def_id");
    ResultSet* rset = stmt->executeQuery();
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("insert into es_sequence_type_def(RUN_TYPE_DEF_ID, SEQUENCE_TYPE_STRING) values "
		 " ( :1, :2 )");
    stmt->setInt(1, run_type_id);
    stmt->setString(2, m_runSeq);
