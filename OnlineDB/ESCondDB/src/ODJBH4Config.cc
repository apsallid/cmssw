...
#include "OnlineDB/ESCondDB/interface/ODJBH4Config.h"
...
int ODJBH4Config::fetchNextId()  throw(std::runtime_error) {
  ...
  try {
    ...
    m_readStmt->setSQL("select es_JBH4_config_sq.NextVal from dual");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO ES_Jbh4_CONFIGURATION ( jbh4_configuration_id, jbh4_tag, "
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT * FROM ES_Jbh4_CONFIGURATION  "
...
int ODJBH4Config::fetchID()    throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT jbh4_configuration_id FROM es_jbh4_configuration "
