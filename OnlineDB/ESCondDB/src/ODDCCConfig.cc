...
#include "OnlineDB/ESCondDB/interface/ODDCCConfig.h"
...
int ODDCCConfig::fetchNextId()  throw(std::runtime_error) {
  ...
  try {
    ...
    m_readStmt->setSQL("select es_dcc_config_sq.NextVal from dual");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO ES_DCC_CONFIGURATION (dcc_configuration_id, dcc_tag, "
    ...
    m_writeStmt = m_conn->createStatement(); 
    m_writeStmt->setSQL ("SELECT dcc_configuration FROM ES_DCC_CONFIGURATION WHERE"
...
int ODDCCConfig::fetchID()    throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT DCC_configuration_id FROM es_dcc_configuration "
