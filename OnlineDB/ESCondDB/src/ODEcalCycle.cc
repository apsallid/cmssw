  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT cycle_id from ES_CYCLE_VIEW "
		 "WHERE sequence_id = :1 ");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT tag, version, sequence_num, cycle_num, cycle_tag, description, "
                     ...
                     "ltc_configuration_id, lts_configuration_id, TTCF_configuration_id, "
                     "srp_configuration_id, \"TTCci_CONFIGURATION_ID\", jbh4_configuration_id, scan_id  "
		 "FROM ES_CYCLE_VIEW where cycle_id = :1 " );
    stmt->setInt(1, id);
    ...
    if (rset->next()) {
      ...
      m_dcc = rset->getInt(8);
      m_ltc = rset->getInt(9);
      m_lts = rset->getInt(10);
      m_ttcf = rset->getInt(11);
      m_srp = rset->getInt(12);
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT d.tag, d.version, d.sequence_num, d.cycle_num, d.cycle_tag, d.description, d.ccs_configuration_id,    "
		       " d.dcc_configuration_id, d.ltc_configuration_id, d.lts_configuration_id, d.ttcf_configuration_id,"
		       " d.srp_configuration_id, d.\"TTCci_CONFIGURATION_ID\" ,  d.jbh4_configuration_id, d.scan_id " 
    ...
    result->setLTCId(        rset->getInt(9) );
    result->setLTSId(        rset->getInt(10) );
    result->setTTCFId(        rset->getInt(11) );
    result->setSRPId(        rset->getInt(12) );
