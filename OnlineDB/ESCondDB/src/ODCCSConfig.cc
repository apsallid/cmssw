...
#include "OnlineDB/ESCondDB/interface/ODCCSConfig.h"

#include <limits>
#define MY_NULL 16777215 // numeric_limits<int>::quiet_NaN()
#define MY_SQL_NULL 16777215 // numeric_limits<int>::quiet_NaN()
...
int ODCCSConfig::fetchNextId()  throw(std::runtime_error) {
  ...
  try {
    ...
    m_readStmt->setSQL("select es_ccs_config_sq.NextVal from dual");
...
int ODCCSConfig::fetchID()    throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT ccs_configuration_id FROM es_ccs_configuration "
