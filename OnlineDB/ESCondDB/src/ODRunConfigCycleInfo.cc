  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO ES_CYCLE_DAT ( sequence_id , cycle_num, tag, description ) "
     "VALUES (:1, :2, :3 , :4 )");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT cycle_id from es_cycle_DAT "
  ...
  throw(std::runtime_error)
{
  ...
  try {
    Statement* stmt = m_conn->createStatement();
    stmt->setSQL("SELECT max(cycle_id) FROM es_cycle_dat "	);
    ResultSet* rset = stmt->executeQuery();
  ...
  throw(std::runtime_error)
{
   ...
   try {
     ...
     stmt->setSQL("SELECT sequence_id , cycle_num , tag , description FROM es_cycle_DAT WHERE cycle_id = :1 ");
     stmt->setInt(1, id);
  ...
  throw(std::runtime_error)
{
  ...
  try {
    m_readStmt->setSQL("SELECT sequence_id , cycle_num , tag , description FROM es_cycle_DAT WHERE cycle_id = :1 ");
