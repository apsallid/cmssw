...
#include "OnlineDB/ESCondDB/interface/ODRunConfigSeqInfo.h"
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT sequence_id from ES_sequence_DAT "
		 "WHERE es_config_id = :id1 " 
  ...
  throw(std::runtime_error)
{
  ...
  try {
    Statement* stmt = m_conn->createStatement();
    stmt->setSQL("SELECT max(sequence_id) FROM es_sequence_dat "	);
    ResultSet* rset = stmt->executeQuery();
  ...
  throw(std::runtime_error)
{
   ...
   try {
     ...
     stmt->setSQL("SELECT es_config_id, sequence_num, num_of_cycles, sequence_type_def_id, description FROM Es_sequence_DAT WHERE sequence_id = :1 ");
     stmt->setInt(1, id);
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO ES_SEQUENCE_DAT ( es_config_id, "
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT es_config_id, sequence_num, num_of_cycles, "
		       "sequence_type_def_id, description FROM Es_sequence_DAT WHERE sequence_id = :1 ");
