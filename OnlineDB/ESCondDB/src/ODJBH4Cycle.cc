  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_writeStmt->setSQL("INSERT INTO ES_JBH4_Cycle (cycle_id, jbh4_configuration_id ) "
		 "VALUES (:1, :2 )");
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    stmt->setSQL("SELECT cycle_id, jbh4_configuration_id FROM es_jbh4_cycle "
		 "WHERE cycle_id = :1 ");
    stmt->setInt(1, m_ID);
  ...
  throw(std::runtime_error)
{
  ...
  try {
    ...
    m_readStmt->setSQL("SELECT  jbh4_configuration_id FROM es_jbh4_cycle "
		 "WHERE cycle_id = :1 ");
